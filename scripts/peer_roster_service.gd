...
func apply_synced_roster(peer_ids: Array[int], internal_ips: Array[String], external_ips: Array[String]) -> void:
	clear()
...
func register_host(host_id: int, internal_ip: String, external_ip: String) -> void:
	clear()
	peer_order.append(host_id)
	upsert_peer(host_id, internal_ip, external_ip)

func ensure_peer_in_order(peer_id: int) -> void:
	if not peer_order.has(peer_id):
		peer_order.append(peer_id)

func upsert_peer(peer_id: int, internal_ip: String, external_ip: String) -> void:
	peer_info_by_id[peer_id] = {
